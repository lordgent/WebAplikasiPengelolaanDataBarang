...
$id = $_GET['id'];
$sel = mysqli_query($conn, "SELECT * FROM tblkondisi WHERE idkondisi = $id");

if(isset($_POST['submit'])) {
      $idkon = $_POST['idkondisi'];
        $kond = $_POST['kondisi'];


    $inst = "UPDATE tblkondisi SET idkondisi = $idkon, kondisi = '$kond' WHERE idkondisi = $id";
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    ...
    <div class="container">
    <br>
    <h3 class="text-center">Edit Kondisi</h3>
   ...
   <?php  foreach($sel as $row) : ?>
    ...
    <input type="hidden" name="idkondisi" value="<?= $row['idkondisi']; ?> ">
  </div>
  <div class="form-group">
    <label>KONFIGURASI KONDISI</label>
    <input type="text" name="kondisi" value=" <?= $row['kondisi']; ?>" class="form-control" placeholder="Contoh : BAIK/RUSAK">
  </div>
