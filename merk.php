...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    ...
    $query = $_POST['namamerk'];
    if($query != '') {

        $dat = mysqli_query($conn, "SELECT * FROM merk WHERE namamerk LIKE '%$query%' ");

    } else{
        
  $dat = mysqli_query($conn, "SELECT * FROM merk");
    ...
    <h5 class="text-center">KONFIGURASI MERK</h5>

   

    <a href="tbhmerk.php" class="btn btn-dark"> 
    <img src="img/plus.png" height="20" alt=""> Tambah Merk
    </a>
  ...
  <?php  $i = 1;   
  ?>
   <?php while( $row = mysqli_fetch_array($dat)) { ?>
    <tr>
      <td scope="row"> <?= $i; ?> </td>
      <td> <?= $row['namamerk']; ?> </td>
      <td>
        <a href="edtmerk.php?idmerk=<?= $row['idmerk']; ?>">
         <img src="img/browser.png" alt="" height="20">
         </a>
         <a href="dltmerk.php?idmerk=<?= $row['idmerk']; ?>" 
         onclick="return confirm('jika kamu menghapus data ini semua data yang berhuungan dengan data ini akan terhaus! buat data baru/lanjut hapus?');"> 
